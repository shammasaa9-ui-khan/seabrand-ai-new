      ...
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {filters.map((filter) => (
              <button
                ...
                className={`px-4 md:px-5 py-2 rounded-xl text-xs md:text-sm font-semibold transition-all cursor-pointer ${
                  activeFilter === filter
                    ? "bg-black text-white"
                    : "bg-zinc-100 text-zinc-500 hover:bg-zinc-200 hover:text-black"
                }`}
        ...
        <div className="flex flex-col">
          {filteredJobs.length > 0 ? (
            filteredJobs.map((job) => {
              ...
              return (
                <div key={job.id} className="border-b border-zinc-100">
                  <div 
                    onClick={() => setExpandedJobId(isExpanded ? null : job.id)}
                    className="group flex flex-col md:flex-row md:items-center justify-between py-6 md:py-8 cursor-pointer hover:px-4 transition-all duration-300 rounded-xl hover:bg-[#FAFAFA]"
                  >
                    <div className="flex flex-col">
                      <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-black mb-2 group-hover:text-[#6748FE] transition-colors">{job.title}</h3>
                      <div className="flex items-center gap-3 md:gap-4 text-zinc-500 text-sm md:text-base font-medium">
                        <span>{job.type}</span>
                        <span className="w-1 h-1 rounded-full bg-zinc-300" />
                        <span>{job.location}</span>
                      </div>
                  ...
                  <AnimatePresence>
                    {isExpanded && (
                      <motion.div
                        ...
                        className="overflow-hidden"
                      >
                        <div className="pb-10 md:pb-12 md:px-4 max-w-3xl">
                          <h4 className="text-xs md:text-sm font-bold text-black mb-3 md:mb-4 uppercase tracking-wider">The Role</h4>
                          <p className="text-zinc-600 leading-relaxed mb-6 md:mb-8 text-base md:text-lg">{job.description}</p>

                          <h4 className="text-xs md:text-sm font-bold text-black mb-3 md:mb-4 uppercase tracking-wider">Requirements</h4>
                          <ul className="list-disc list-inside text-zinc-600 space-y-2.5 md:space-y-3 mb-8 md:mb-10 text-base md:text-lg">
                            {job.requirements.map((req: string, i: number) => (
                              <li key={i}>{req}</li>
                            ))}
                          </ul>
                          ...
                          <button
                            onClick={(e) => handleApplyClick(e, job.id)}
                            className="group relative overflow-hidden bg-black text-white px-7 md:px-8 py-3.5 md:py-4 rounded-xl text-xs md:text-sm font-bold"
                          >
                            <span className="relative z-10 flex items-center gap-2">
                              Apply Now <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                            </span>
            ...
            <div className="py-20 text-center text-zinc-500 font-medium text-sm md:text-base">
              No open positions found for this work style currently.
            </div>
          )}
        </div>
