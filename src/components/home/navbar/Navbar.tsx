...
export default function Navbar() {
  ...
  elements =
    ...
    // Backup interval check for lazy loads / dynamic content rendering
  ...
  // Force light (dark text) when mobile menu overlay is active
  const activeTheme = isOpen ? 'light' : theme;

  return (
    <>
      <header role="banner" className={`fixed top-4 md:top-6 left-1/2 -translate-x-1/2 w-[92%] max-w-7xl z-50 ${activeTheme === 'dark' ? 'bg-black/20' : 'bg-white/20'} backdrop-blur-lg border border-white shadow-[0_5px_30px_rgba(0,0,0,0.15)] rounded-2xl transition-colors duration-500 ${activeTheme === 'dark' ? 'text-white' : 'text-black'}`}>
        <div className="flex items-center justify-between py-4 px-6 md:px-8">
          ...
          <div className="hidden md:flex items-center gap-6">
            <button className="flex items-center gap-1 text-sm font-medium hover:text-[#6748FE] transition-colors" aria-label="Change Language, Current language English">
